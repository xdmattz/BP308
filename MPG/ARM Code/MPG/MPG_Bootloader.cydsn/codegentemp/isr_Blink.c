...
{
    #ifdef isr_Blink_INTERRUPT_INTERRUPT_CALLBACK
        isr_Blink_Interrupt_InterruptCallback();
    #endif /* isr_Blink_INTERRUPT_INTERRUPT_CALLBACK */ 
...
void isr_Blink_SetVector(cyisraddress address)
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + (uint32)isr_Blink__INTC_NUMBER] = address;
}
...
cyisraddress isr_Blink_GetVector(void)
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + (uint32)isr_Blink__INTC_NUMBER];
}
...
{
    *isr_Blink_INTC_PRIOR = priority << 5;
}
...
uint8 isr_Blink_GetPriority(void)
{
    ...
    priority = *isr_Blink_INTC_PRIOR >> 5;
